...
<body class="bg-gray-100">
    <div class="container mx-auto py-8">
        ...
        @endif
        <form action="{{ route('designs.update', $design->id) }}" method="POST" enctype="multipart/form-data" class="max-w-lg mx-auto">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="nama" class="block text-gray-700 text-sm font-bold mb-2">Nama:</label>
                <input type="text" name="nama" id="nama" class="form-input w-full" value="{{ $design->nama }}">
            </div>
            <div class="mb-4">
                <label for="tanggal" class="block text-gray-700 text-sm font-bold mb-2">Tanggal:</label>
                <input type="date" name="tanggal" id="tanggal" class="form-input w-full" value="{{ $design->tanggal }}">
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="text" id="text" class="form-textarea w-full">{{ $design->text }}</textarea>
            </div>
